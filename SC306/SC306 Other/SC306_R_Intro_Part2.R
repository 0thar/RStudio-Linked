...
## R has special values for infinity (Inf), not-a-number (NaN), 
## and not-applicable (NA). 
## These generally behave very sensibly - a mathematical operation on
...
## determining whether x is a valid number:  is.na(x)

## It is worth noting that information about these
...
## (make sure you understand what's happening above - if you don't - ASK!)
...
## numbers. Most counting can be accomplished by use of the seq()
## function. If you haven't already done so, it is worth taking a look at
## the help-file on seq() because it has a few arguments that can make your 
## life easier.
...
## 1:2 or 2:1, respectively.  You may want to check out the
...
## dataset. Each item in a dataset has it's own index, or numbered
...
## The first option: Logical vector of length n
...
## The second option: Value or vector of any length with values
## (1 to n) OR (-n to 1) 
...
## Note that it returns the indices in the right order (alphbetically), 
...
## analogous, one way to plot the result is:
...
###################################################################### 
## Before we cover advanced topics of data frames, I wanted to point 
## out the function data.frame() which puts data together to form data
## frames. 
...
## Indexing data frames:  brackets, logical, or
## numerical vectors are still the way to access data frames, but with
## a slight complication, because data frames are
## multidimensional. The solution (which also holds for matrices,
## etc.) is to separate the two dimensions with a comma. R treats the
## first entry as the row number and the second entry as the column
## number; thus, to access the second column of the fourth row, type
...
## Introduction to factors When performing statistical analyses - we
## often want R to look at a set of data and compare groups within the
## data to one another. For example, you have our data frame
...
## The answer to these questions is to use factors.  You can think
## of factors as categorical variables.

## Many of the datasets that come with R already have their data
## interpreted as factors. Let's take a look at a dataset with
## factors:

## Note that you may have to install  and load the DAAG package in order to
## load these data.  To do so, in Rstudio click on the packages tab 
## then click on Install Packages and type in DAAG in the window that
## appears and click Install.  Verify that the DAAG package now appears
## in your list of installed packages.  Check it's box to load it.
...
## The help file tells us that our last column,
...
# Factors will be exceptionally handy when performing statistical
...
## Making a factor: Now you know how to employ a factored variable, and
...
## So far, we have been using data contained within R's
## packages; however, when working on your own data/research you will most
...
## (note: you may have to minimize your current window to see the file.choose
## window that pops up!)
##
## This will let you use yoursystem's familiar file-selection window
## to locate the data on your drive. Note: the sep = "" argument specifies
## that white space deliniates your data.  If this is not the case, you'll  
## have to specify the correct deliniation.  For example, if you have data
## stored in a spreadsheet (e.g. excel), you can save the spreadsheet as a
## csv file (comma separated values) and then specifiy sep = "," to read it
## into R
...
## A few work nicely: Here we access the built in airquality data in R
## (you can type ?airquality to get more details)
...
###################################################################### 
## A more advanced (and very important) topic - So far in R we have used
## the functions that come with R and its various packages. You have
## come up with methods for adjusting your data for visualization on
## your own, but you did this in many separate steps, each of which
## refer to the specific items you are manipulating. Since often you
## want to perform the same series of actions on different objects, R
## makes it relatively easy to compose your own generic functions and
...
## looks like a function but is not a function. What is a function? It's
...
## above) but this can make it difficult to follow what is going on 
## when you return to the code later on. While
...
## SUM.VALS.PLUS.SUM.SQS() - a function that takes two numerical values
## as input and returns the sum of the values plus the sum of their
## squares:

sum.vals.plus.sum.sqs <- function(x,y)
{
  z <- x^2 + y^2        # define z as the sum of the values's squares
